...
from permea_core.validation import artifacts


REQUIRED_STEP_NAMES = tuple(name for name, _command in artifacts.VALIDATION_STEPS)


def test_unified_validation_passes_current_repo_state() -> None:
    result = artifacts.validate_permea_artifacts()
...
def test_report_includes_each_required_step_name() -> None:
    result = {
        ...
        "steps": [
            ...
            for name, command in artifacts.VALIDATION_STEPS
        ],
    }

    report = artifacts.format_validation_report(result)

    assert "PASS Permea artifact validation" in report
...
def test_failed_subprocess_step_is_reported_as_fail(monkeypatch) -> None:
    ...
    step = artifacts.run_command_step(
...
def test_cli_exits_zero_on_current_repo_state() -> None:
    completed = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "validate_permea_artifacts.py")],
        cwd=ROOT,
        check=False,
        text=True,
        capture_output=True,
    ...
    assert "PASS Permea artifact validation" in completed.stdout
    for name in REQUIRED_STEP_NAMES:
